...
from .config import (kisa_ad, CHROMA_YOLU, KOLEKSIYON, EMBED_MODEL, RERANK_MODEL, SORGU_CIHAZ,
...
# Uygulamanin RAG yolu tamamen yerel calisir. Modeller kurulum sirasinda
# indirilmis olmalidir; calisma zamaninda Hugging Face'e ustveri/model istegi
# gonderilmez. Yerel model yoksa agda tekrar denemek yerine hemen hata verir.
...
def _reranker():
    ...
    try:
        return CrossEncoder(RERANK_MODEL, device=RERANK_CIHAZ,
                            max_length=RERANK_MAXLEN, local_files_only=True)
...
def ara(soru: str, k: int = TOP_K_CEVAP, k_arama: int = TOP_K_ARAMA,
    ...
    if not adaylar and where is not None:
        return ara(soru, k, k_arama, rerank, doc_basina, filtre_kullan=False)
...
def veritabani_baglami_olustur(slot: dict) -> str:
    ...
    for r in satirlar:
        ...
        satirlar = [f"• {kisa_ad(r['banka_adi'])} | {URUN_ETIKET.get(r['urun_tipi'], r['urun_tipi'])}:"]
        if r['oran'] is not None:
            satirlar.append(f"  - Kesin Kâr Payı Oranı: %{r['oran']}")
        if r['vade']:
            satirlar.append(f"  - Azami Vade: {r['vade']} Ay")
        if r.get('tutar'):
            _t = r['tutar']
            _ts = f"{int(_t):,}".replace(",", ".") if _t == int(_t) else str(_t)
            satirlar.append(f"  - Azami Finansman Tutarı: {_ts} TL")
        tahsis_ham = r.get('tahsis')
        if tahsis_ham and ('0.50' in str(tahsis_ham) or '0,50' in str(tahsis_ham)):
            satirlar.append("  - Tahsis Ücreti: %0.50")
        elif tahsis_ham and len(str(tahsis_ham).strip()) <= 25:
            satirlar.append(f"  - Tahsis Ücreti: {str(tahsis_ham).strip()}")
        hk_ham = r.get('hedef_kitle')
        if hk_ham:
            hk_parcalar = [q.strip() for q in hk_ham.split(',') if q.strip()]
            satirlar.append("  - Hedef Kitle: " + ", ".join(dict.fromkeys(hk_parcalar)))
        blok.append("\n".join(satirlar))
    return "\n".join(blok)
